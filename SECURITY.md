...
- Azure deployment uses service-principal credentials from protected GitHub
  Environment secrets. Scope the principal's Azure role to the intended App Service
  resources and rotate `ARM_CLIENT_SECRET` regularly.
...
- The personal GitHub staging mirror excludes `.github/` so copied workflow tokens,
  deployment jobs, and Dependabot automation cannot execute there.
...
Store `ARM_CLIENT_ID`, `ARM_CLIENT_SECRET`, `ARM_TENANT_ID`, and
`ARM_SUBSCRIPTION_ID` as protected environment secrets; never commit them.
