...
The repository now supports both GitHub Actions and Azure DevOps deployment automation. The main GitHub Actions workflow and the Azure DevOps ZIP-deploy pipeline both build one ZIP package and can deploy it to up to three Linux App Service targets. The current default path uses Azure CLI ZIP deploy plus App Service Oryx build automation, not `WEBSITE_RUN_FROM_PACKAGE`.
...
5. Check primary, secondary, and third App Service targets before deployment
