...
MAX_EASY_AUTH_HEADER_BYTES = 16_384
...
def create_app() -> Flask:
    app = Flask(__name__)
    is_app_service = bool(os.getenv("WEBSITE_SITE_NAME", "").strip())
    trust_proxy_headers = is_app_service or env_flag("TRUST_PROXY_HEADERS")
    ...
    if not secret_key:
        if is_app_service:
            raise RuntimeError("FLASK_SECRET_KEY must be configured on Azure App Service.")
    ...
    app.config["TRUST_EASY_AUTH_HEADERS"] = is_app_service or env_flag("TRUST_EASY_AUTH_HEADERS")
    ...
    app.config["AAD_SCOPES"] = [
        scope.strip() for scope in os.getenv("AAD_SCOPES", "User.Read").split(",") if scope.strip()
    ]
...
def register_routes(app: Flask) -> None:
    ...
    def login_easyauth():
        ...
        post_login_redirect_uri = urljoin(request.host_url, url_for("profile_easyauth").lstrip("/"))
    ...
    def authorized():
        ...
        result = build_msal_app(app).acquire_token_by_auth_code_flow(auth_flow, request.args)
...
def build_app_service_portal_url(app: Flask) -> str:
    ...
    subscription_id = (
        parse_subscription_id(app.config["APP_SERVICE_SUBSCRIPTION_ID"])
        or os.getenv("APP_SERVICE_SUBSCRIPTION_ID", "").strip()
        or os.getenv("ARM_SUBSCRIPTION_ID", "").strip()
    )
...
def get_runtime_info() -> list:
    ...
    version = f"Python {python_version.major}.{python_version.minor}.{python_version.micro}"
...
def get_easy_auth_user() -> dict:
    if not current_app.config["TRUST_EASY_AUTH_HEADERS"]:
        return {}

    principal_header = request.headers.get("X-MS-CLIENT-PRINCIPAL", "")
    if not principal_header or len(principal_header) > MAX_EASY_AUTH_HEADER_BYTES:
        return {}

    try:
        payload = base64.b64decode(principal_header, validate=True)
    ...
    return user if any(str(user.get(key) or "").strip() for key in identity_fields) else {}
