...
def test_home_renders_expected_navigation(client):
    ...
    assert "Identity Demo" in html
    assert "Auto Sign-out" not in html


def test_msal_session_hides_all_sign_in_actions(client):
    with client.session_transaction() as flask_session:
        flask_session["msal_user"] = {"name": "Demo User"}

    response = client.get("/")
    html = response.get_data(as_text=True)

    assert response.status_code == 200
    assert "View MSAL profile" in html
    assert "Sign out (MSAL)" in html
    assert "mode-signin-button" not in html
...
def test_health_endpoint_and_security_headers(client):
    ...
    compatibility_response = client.get("/healthz", follow_redirects=False)
    assert compatibility_response.status_code == 200
    assert compatibility_response.json == {"status": "ready"}
    assert "Location" not in compatibility_response.headers
